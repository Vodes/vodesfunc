...
def vodes_rescale(src: vs.VideoNode, height: float = 0, width: float = None, 
    mode: int = 0, descale_kernel: Kernel | List[vs.VideoNode] = Catrom(), downscaler: Kernel | str = Catrom(), opencl: bool = True, 
    modeargs: dict = {"radius": 2, "strength": 50}, ediargs: dict = {"qual": 2, "nsize": 4, "nns": 4, "pscrn": 1}, 
    ...
    from vsutil import get_y, depth, get_depth, iterate, Range
    ...
    ref_y = descale_kernel.scale(descaled_y, src.width, src.height) if isinstance(descale_kernel, Kernel) else descale_kernel[1]
    ...
    if mode == 0:
        ...
        if "ratio" in modeargs:
            doubled_y = core.std.Expr([nnedi, fsrcnnx], f"{modeargs.get('ratio')} x * {1 - modeargs.get('ratio')} y * +")
        else:
            doubled_y = fsrcnnx
    ...
    out = rescaled_y if clip.format.color_family == vs.GRAY else core.std.ShufflePlanes([rescaled_y, clip], [0, 1, 2], vs.YUV)
    ref_out = ref_y if clip.format.color_family == vs.GRAY else core.std.ShufflePlanes([ref_y, clip], [0, 1, 2], vs.YUV)
...
def nnedi_double(clip: vs.VideoNode, opencl: bool = True, ediargs: dict = {"qual": 2, "nsize": 4, "nns": 4, "pscrn": 1}) -> vs.VideoNode:
    """
    ...
    """
    y = get_y(clip)
    doubled_y = y.nnedi3cl.NNEDI3CL(dh = True, field = 0, **ediargs).std.Transpose().nnedi3cl.NNEDI3CL(dh = True, field = 0, **ediargs).std.Transpose() if opencl \
        else y.znedi3.nnedi3(dh = True, field = 0, **ediargs).std.Transpose().znedi3.nnedi3(dh = True, field = 0, **ediargs).std.Transpose()
